...
{
public:
	ConHexWireScene() : Scene("Concave Hexahedron Filled Wireframe") {}
	...
	virtual void Draw(Graphics& gfx) const override
	{
		...
		{
			gfx.DrawTriangle(triangles.vertices[triangles.indices[i * 3]], triangles.vertices[triangles.indices[i * 3 + 1]], triangles.vertices[triangles.indices[i * 3 + 2]], colorSolid);
		}
		// draw the lines
		for (size_t i = 0, end = lines.indices.size() / 2; i < end; i++)
		{
			gfx.DrawLine(lines.vertices[lines.indices[i * 2]], lines.vertices[lines.indices[i * 2 + 1]], colorWire);
		}
	}
private:
