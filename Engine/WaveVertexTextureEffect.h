...
public:
	...
	public:
		...
		Vertex(const Vec3& pos, const Vertex& src) : t(src.t), pos(pos) {}
		Vertex(const Vec3& pos, const Vec2& t) : t(t), pos(pos) {}
	...
	public:
		...
		public:
			...
			Output(const Vec3& pos, const Output& src) : t(src.t), l(src.l), pos(pos) {}
			Output(const Vec3& pos, const Vec2& t, float l) : t(t), l(l), pos(pos) {}
	...
	public:
		...
		Color operator()(const Input& in) const
		{
			...
			const Vec3 color = Vec3(pTex->GetPixel((unsigned int)std::min(in.t.x * tex_width + 0.5f, tex_xclamp), (unsigned int)std::min(in.t.y * tex_height + 0.5f, tex_yclamp)));
