...
{
public:
	TexCubeScene() : TexCubeScene(1.0f) {}
	TexCubeScene(float texdim) : cube(1.0f, texdim), Scene("Textured Cube Butthole Of Sauron Clamped Dim: " + std::to_string(texdim)) {}
	...
	virtual void Draw(Graphics& gfx) const override
	{
		...
		for (size_t i = 0, end = triangles.indices.size() / 3; i < end; i++)
		{
			...
			{
				gfx.DrawTriangleTex(triangles.vertices[triangles.indices[i * 3]], triangles.vertices[triangles.indices[i * 3 + 1]], triangles.vertices[triangles.indices[i * 3 + 2]], sbTex);
			}
		}
	}
private:
