...
public:
	...
	public:
		...
		Vertex(const Vec3& pos, const Vertex& src) : n(src.n), pos(pos) {}
		Vertex(const Vec3& pos, const Vec3& n) : n(n), pos(pos) {}
	public:
		Vec3 pos;
		Vec3 n;
	};
	...
	public:
		...
		public:
			Output() = default;
			Output(const Vec4& pos) : pos(pos) {}
			Output(const Vec4& pos, const Output& src) : n(src.n), worldPos(src.worldPos), pos(pos) {}
			Output(const Vec4& pos, const Vec3& n, const Vec3& worldPos) : n(n), pos(pos), worldPos(worldPos) {}
	...
	public:
		...
		Color operator()(const Input& in) const
		{
			...
			const auto attenuation = 1.0f / (constant_attenuation + linear_attenuation * dist + quadradic_attenuation * sq(dist));
